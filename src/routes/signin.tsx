...
export const Route = createFileRoute('/signin')({
  ...
  beforeLoad: ({ context, search }) => {
    // customLog('signin-auth-status:', context.auth);
...
function isMfaChallenge(response: unknown): response is MfaChallengeResponse {
  if (response && typeof response === 'object' && response !== null) {
    if ('mfa' in response && 'mfaId' in response) {
      return response.mfa === true && typeof response.mfaId === 'string';
    }
  }
  return false;
}
...
function OtpInputForm({
  ...
  return (
    ...
    >
      ...
      <div>
        <SubmitButton type="submit" disabled={isSubmitting || isTimerExpired}>
...
function OtpRequestForm({ onSubmit, isSubmitting, error }: OtpRequestFormProps) {
  const [email, setEmail] = useState('');
...
function UserAuthForm({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) {
  ...
  const renderForm = () => {
    switch (formState) {
      case 'signIn':
        return (
          <UserSignInForm
            ...
            onMfaRequired={(mfaId, email) => {
              void handleMfaRequiredForSignIn(mfaId, email);
            }}
          />
        );
      case 'signUp':
        return (
          <UserSignUpForm
            ...
            onMfaRequired={(mfaId, email) => {
              void handleMfaRequiredForSignUp(mfaId, email);
            }}
          />
        );
      case 'mfaSignIn':
      ...
      case 'otpRequest':
        return (
          <OtpRequestForm onSubmit={handleRequestOtp} isSubmitting={isSubmitting} error={error} />
        );
